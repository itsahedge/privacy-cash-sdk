...
export class PrivacyCash {
   ...
   */
  async deposit({ lamports }: { lamports: number }) {
    this.isRuning = true;
  ...
  async withdraw({
    ...
    console.log(
      `Withdraw successful. Recipient ${recipient} received ${
        res.amount_in_lamports / LAMPORTS_PER_SOL
      } SOL, with ${res.fee_in_lamports / LAMPORTS_PER_SOL} SOL relayers fees`
    );
    this.isRuning = false;
    return res;
  }
