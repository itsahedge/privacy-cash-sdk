...
export async function deposit({
  ...
  logger.debug(
    `Deposit amount: ${amount_in_lamports} lamports (${
      amount_in_lamports / LAMPORTS_PER_SOL
    } SOL)`
  );
  logger.debug(
    `Calculated fee: ${fee_amount_in_lamports} lamports (${
      fee_amount_in_lamports / LAMPORTS_PER_SOL
    } SOL)`
  );
  ...
  if (balance < amount_in_lamports + fee_amount_in_lamports) {
    throw new Error(
      `Insufficient balance: ${balance / 1e9} SOL. Need at least ${
        (amount_in_lamports + fee_amount_in_lamports) / LAMPORTS_PER_SOL
      } SOL.`
    );
  }
...
export async function depositV2({
  ...
  logger.debug(`User wallet: ${publicKey.toString()}`);
  logger.debug(
    `Total amount: ${totalLamports} lamports (${
      totalLamports / LAMPORTS_PER_SOL
    } SOL)`
  );
  logger.debug(
    `Fee to recipient: ${feeLamports} lamports (${
      feeLamports / LAMPORTS_PER_SOL
    } SOL)`
  );
  logger.debug(
    `Actual deposit amount: ${amount_in_lamports} lamports (${
      amount_in_lamports / LAMPORTS_PER_SOL
    } SOL)`
  );
  ...
  if (balance < totalLamports + txFeeBuffer) {
    throw new Error(
      `Insufficient balance: ${balance / 1e9} SOL. Need at least ${
        (totalLamports + txFeeBuffer) / LAMPORTS_PER_SOL
      } SOL.`
    );
  }
  ...
  if (feeLamports > 0) {
    ...
    logger.debug(
      `Added fee transfer instruction: ${feeLamports} lamports to ${feeRecipient.toString()}`
    );
  }
