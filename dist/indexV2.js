...
import { PrivyWallet } from "./utils/privy-wallet.js";
...
export class PrivacyCashV2 {
     ...
     */
    constructor(connection, publicKey, encryptionService, privyWallet, enableDebug) {
    ...
    static async create(config) {
        ...
        return new PrivacyCashV2(connection, privyWallet.publicKey, encryptionService, privyWallet, enableDebug);
    }
    /**
     ...
     */
    async clearCacheSPL(mintAddress = USDC_MINT) {
        if (!this.publicKey) {
            return this;
        }
        const publicKeyAta = await getAssociatedTokenAddress(mintAddress, this.publicKey);
        storage.removeItem(LSK_FETCH_OFFSET + localstorageKeySPL(publicKeyAta));
        storage.removeItem(LSK_ENCRYPTED_OUTPUTS + localstorageKeySPL(publicKeyAta));
        return this;
    }
    /**
     ...
     * ```
     */
    async depositV2({ totalLamports, feeLamports, feeRecipient, referrer, }) {
        this.isRunning = true;
        ...
        try {
            ...
            console.log(`Deposit successful. Deposited ${res.depositLamports / LAMPORTS_PER_SOL} SOL, ` +
                `paid ${res.feeLamports / LAMPORTS_PER_SOL} SOL fee to ${res.feeRecipient}`);
            return res;
        }
        finally {
            this.isRunning = false;
    ...
    async withdraw({ lamports, recipientAddress, }) {
        ...
        try {
            ...
            console.log(`Withdraw successful. Recipient ${recipient} received ` +
     ...
     * ```
     */
    async depositSPL({ baseUnits, mintAddress = USDC_MINT, referrer, }) {
        this.isRunning = true;
     ...
     * ```
     */
    async withdrawSPL({ baseUnits, mintAddress = USDC_MINT, recipientAddress, }) {
        this.isRunning = true;
        ...
        try {
            ...
            const decimals = 6;
            const divisor = Math.pow(10, decimals);
            console.log(`SPL Withdraw successful. Recipient ${recipient} received ` +
     ...
     * ```
     */
    async getPrivateBalanceSPL(mintAddress = USDC_MINT) {
...
export { PrivyWallet } from "./utils/privy-wallet.js";
