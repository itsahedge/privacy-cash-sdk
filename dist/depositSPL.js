...
import { ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, getAssociatedTokenAddressSync, getMint, getAccount, createTransferInstruction, } from "@solana/spl-token";
...
export async function depositSPLV2({ lightWasm, storage, keyBasePath, publicKey, connection, totalBaseUnits, feeBaseUnits, feeRecipient, encryptionService, transactionSigner, referrer, mintAddress, }) {
    ...
    try {
        ...
        throw new Error(`Fee recipient ${feeRecipient.toString()} does not have a token account for mint ${mintAddress.toString()}. ` +
