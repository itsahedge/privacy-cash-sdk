...
export async function depositV2({ lightWasm, storage, keyBasePath, publicKey, connection, totalLamports, feeLamports, feeRecipient, encryptionService, transactionSigner, referrer, }) {
    // Calculate actual deposit amount after fee
    const amount_in_lamports = totalLamports - feeLamports;
    if (amount_in_lamports <= 0) {
        throw new Error("Total lamports must be greater than fee lamports");
    }
    if (feeLamports < 0) {
        throw new Error("Fee lamports must be non-negative");
    }
    // Check deposit limit
    let limitAmount = await checkDepositLimit(connection);
    if (limitAmount && amount_in_lamports > limitAmount * LAMPORTS_PER_SOL) {
    ...
    const fee_amount_in_lamports = 0; // Protocol fee (separate from user fee)
    logger.debug("Encryption key generated from user keypair");
    logger.debug(`User wallet: ${publicKey.toString()}`);
    logger.debug(`Total amount: ${totalLamports} lamports (${totalLamports / LAMPORTS_PER_SOL} SOL)`);
    logger.debug(`Fee to recipient: ${feeLamports} lamports (${feeLamports / LAMPORTS_PER_SOL} SOL)`);
    logger.debug(`Actual deposit amount: ${amount_in_lamports} lamports (${amount_in_lamports / LAMPORTS_PER_SOL} SOL)`);
    logger.debug(`Fee recipient: ${feeRecipient.toString()}`);
    // Check wallet balance (need total + tx fees)
    const balance = await connection.getBalance(publicKey);
    logger.debug(`Wallet balance: ${balance / 1e9} SOL`);
    // Need enough for totalLamports (which includes fee) plus some buffer for tx fees
    const txFeeBuffer = 0.01 * LAMPORTS_PER_SOL; // ~0.01 SOL buffer for transaction fees
    if (balance < totalLamports + txFeeBuffer) {
        throw new Error(`Insufficient balance: ${balance / 1e9} SOL. Need at least ${(totalLamports + txFeeBuffer) / LAMPORTS_PER_SOL} SOL.`);
    }
    const { treeAccount, treeTokenAccount, globalConfigAccount } = getProgramAccounts();
    // Create the merkle tree with the pre-initialized poseidon hash
    const tree = new MerkleTree(MERKLE_TREE_DEPTH, lightWasm);
    ...
    // Create fee transfer instruction (if fee > 0)
    const instructions = [
    ...
    if (feeLamports > 0) {
        ...
        logger.debug(`Added fee transfer instruction: ${feeLamports} lamports to ${feeRecipient.toString()}`);
    }
    ...
    while (true) {
        ...
        if (resJson.exists) {
            ...
            return {
                tx: signature,
                depositLamports: amount_in_lamports,
